...
public class DataCentreService(IDbContextFactory<ClubContext> contextFactory) : IDataCentreService
{
    ...
    public async Task<List<YearCountVO>> GetYearDataAsync()
    {
        ...
        // 添加历史学年数据
        yearData.AddRange([
            new YearCountVO("2019学年", 33),
            new YearCountVO("2020学年", 1),
            new YearCountVO("2021学年", 274),
            new YearCountVO("2022学年", 329)
        ]);
        ...
        for (var i = year - 2024; i >= 0; i--)
        {
            ...
            yearData.Add(new YearCountVO($"{year - i - 1}学年", v));
        }
        ...
        yearData.Add(new YearCountVO($"{year}学年", value));
    ...
    public async Task<List<GradeCountVO>> GetGradeDataAsync()
    {
        ...
        var gradeData = groupedStudents.Select(group => new GradeCountVO(group.Key + "级", group.Count())).ToList();
    ...
    public async Task<List<LandscapeCountVO>> GetLandscapeDataAsync()
    {
        ...
        return groupedStudents.Select(group => new LandscapeCountVO(group.Key, group.Count())).ToList();
    }
