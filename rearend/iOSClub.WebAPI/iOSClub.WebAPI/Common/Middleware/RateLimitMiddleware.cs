...
/// </summary>
public class RateLimitMiddleware(RequestDelegate next, ILogger<RateLimitMiddleware> logger, RateLimitService rateLimitService)
{
    ...
    // 请求来源黑名单（可从配置或数据库加载）
    private readonly HashSet<string> _ipBlacklist = new HashSet<string>();
    
    // 异常请求检测阈值
    private const int SuspiciousRequestThreshold = 20; // 每分钟超过20个请求视为可疑
    
    // 可疑请求计数器（IP -> 时间戳列表）
    private readonly Dictionary<string, List<DateTime>> _suspiciousRequestTracker = new Dictionary<string, List<DateTime>>();
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        // 4. 尝试获取令牌
        var lease = await _rateLimitService.TryAcquireTokenAsync(policy, CancellationToken.None);
