    ...
    : ControllerBase
{
    ...
    /// </summary>
    [HttpGet("{name?}")]
    public async Task<ActionResult<ApiResponse<DepartmentDO>>> GetDepartment(string? name)
    {
        try
        {
            var userJwt = httpContextAccessor.HttpContext?.User.GetUser();
            if (userJwt == null)
                return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.Unauthorized, "用户未认证"));

            // 检查用户权限
            if (!IsAuthorizedUser(userJwt.Identity))
                return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.InsufficientPermission, "权限不足"));

            var staff = await staffRepository.GetStaffByIdAsync(userJwt.UserId);
            if (staff == null) return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.Unauthorized, "用户未找到或权限不足"));
            // 如果没有指定部门名称，返回用户所在部门
            if (string.IsNullOrEmpty(name))
            {
                var userDepartment = await departmentRepository.GetDepartmentByNameAsync(staff.Department?.Name ?? "");
                if (userDepartment == null)
                    return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.ResourceNotFound, "用户未分配部门"));

                // 普通用户只能查看自己部门
                if (!IsAdminUser(userJwt.Identity) && userDepartment.Name != staff.Department?.Name)
                    return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.InsufficientPermission, "只能查看自己部门的信息"));

                return Ok(ApiResponse<DepartmentDO>.Success(userDepartment));
            }
            ...
            if (IsAdminUser(userJwt.Identity))
            {
                ...
                if (department == null)
                    return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.ResourceNotFound, "部门不存在"));

                return Ok(ApiResponse<DepartmentDO>.Success(department));
            }
            ...
            if (userDept == null)
            {
                return NotFound(ApiResponse<DepartmentDO>.Fail(ErrorCode.ResourceNotFound, "部门不存在"));
            }

            return Ok(ApiResponse<DepartmentDO>.Success(userDept));
        }
        catch (Exception ex)
        {
            ...
            return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.InternalServerError, $"服务器错误: {ex.Message}"));
        }
    }
    ...
    public async Task<ActionResult<ApiResponse<List<DepartmentDO>>>> GetAllDepartments()
    {
        try
        {
            var departments = await departmentRepository.GetAllDepartmentsAsync();
            return Ok(ApiResponse<List<DepartmentDO>>.Success(departments));
        }
        catch (Exception ex)
        {
            if (logger.IsEnabled(LogLevel.Information))
            ...
            return Ok(ApiResponse<List<DepartmentDO>>.Fail(ErrorCode.InternalServerError, $"服务器错误: {ex.Message}"));
        }
    }
    ...
    [HttpPost("Update")]
    [Authorize(Roles = "Founder,President,Minister")]
    public async Task<ActionResult<ApiResponse<string>>> UpdateDepartment([FromBody] DepartmentDO model)
    {
        try
        {
    ...
    public async Task<ActionResult<ApiResponse<DepartmentDO>>> CreateDepartment([FromBody] DepartmentDO model)
    {
        try
        {
            // 检查部门是否已存在
            ...
            if (existingDept != null)
                return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.ResourceAlreadyExists, "部门已存在"));

            var result = await departmentRepository.AddDepartmentAsync(model);
            if (!result)
                return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.OperationFailed, "创建失败"));

            return CreatedAtAction(nameof(GetDepartment), new { name = model.Name },
                ApiResponse<DepartmentDO>.Success(model, "部门创建成功"));
        }
        catch (Exception ex)
        {
            if (logger.IsEnabled(LogLevel.Information))
            ...
            return Ok(ApiResponse<DepartmentDO>.Fail(ErrorCode.InternalServerError, $"创建失败: {ex.Message}"));
        }
    }
